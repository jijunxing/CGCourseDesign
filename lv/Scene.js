...
export default class Scene{
  ...
  updateChildren2Draw() {
    ...
    children.forEach(child => {
      const name = child.mat.programName
  ...
  draw(uniforms) {
    ...
    for (let [key, objs] of children2Draw.entries()) {
      ...
      const { program, attributes, uniforms: programUniforms } = programs.get(key)
      gl.useProgram(program)
      objs.forEach(obj => {
        if (!obj.geo || !obj.mat) return;
        const { geo: { drawType,count},mat:{mode}}=obj
        obj.update(gl,attributes,programUniforms)
